...
export function Room(): React.JSX.Element {
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-[#7A7D7D]">
      {/* Sidebar */}
      <div
        className={`${isSidebarOpen ? "w-64" : "w-0"} transition-all duration-300 overflow-hidden bg-gray-50 border-r border-gray-200 flex flex-col dark:bg-[#7A7D7D]`}
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-[#7A7D7D] text-gray-900 dark:text-white">
          <div className="max-w-3xl mx-auto px-4">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`py-6 ${message.type === "user" ? "place-items-end" : "place-items-start"}`}
              >
